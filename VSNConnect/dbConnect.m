...
-(void)addStatus:(NSString *)bledate bleName:(NSString *)bleName  bleAddress:(NSString *)bleAddress bleStatus:(NSString *)bleStatus
{
    ...
    sql = [[NSString stringWithFormat:@"insert into history(dates,devicename,macaddress,status) values ('%@','%@','%@','%@')",bledate,bleName,bleAddress,bleStatus] UTF8String];
...
-(void)updatestatus:(NSString *)fieldName value:(NSString *)FieldValue mac:(NSString *)StrId
{
    ...
    if(sqlite3_open([documentsPath UTF8String], &database) == SQLITE_OK)
    {
        ...
        NSLog(@"%@",FieldValue);
        NSLog(@"%@", [NSString stringWithFormat:@"update history set %@ = '%@'  where macaddress = '%@'",fieldName,FieldValue,StrId]);
...
-(void)updatedeviceinfo:(NSString *)fieldName value:(NSString *)FieldValue mac:(NSString *)StrId
{
    ...
    if(sqlite3_open([documentsPath UTF8String], &database) == SQLITE_OK)
    {
        ...
        NSLog(@"%@",FieldValue);
        NSLog(@"%@", [NSString stringWithFormat:@"update deviceinfo set %@ = '%@'  where macaddress = '%@'",fieldName,FieldValue,StrId]);
        if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK)
        {
            NSLog(@"Sucessfully Updated");
        }
    ...
    if(sqlite3_open([documentsPath UTF8String], &database) == SQLITE_OK)
    {
        ...
        if(sqlite3_prepare_v2(database, sqlStatement, -1, &compiledStatement, NULL) == SQLITE_OK)
        {
            ...
            while(sqlite3_step(compiledStatement) == SQLITE_ROW)
            {
                ...
                NSLog(@"already presented? - %d",count);
                //count = 1;
            }
        }
    }
